...
interface HeroImageSummaryTableProps {
  data: SKUData[];
  filters?: FilterState;
...
export const HeroImageSummaryTable = ({ data, filters, onDrillDown }: HeroImageSummaryTableProps) => {
  ...
  const calculateHeroImageScores = (skus: SKUData[]) => {
    const overallTitleScore = Math.round(skus.reduce((sum, sku) => sum + sku.heroImage.score.overall, 0) / skus.length);
    const lengthScore = Math.round(skus.reduce((sum, sku) => sum + (sku.heroImage.imageClarity === 'clear' ? 100 : sku.heroImage.imageClarity === 'moderate' ? 70 : 40), 0) / skus.length);
    const structureScore = Math.round(skus.reduce((sum, sku) => sum + (sku.heroImage.optimalSizing ? 100 : 50), 0) / skus.length);
    const searchTermsScore = Math.round(skus.reduce((sum, sku) => sum + (sku.heroImage.isCorrectImage ? 100 : 0), 0) / skus.length);
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            <TableHead className="w-1/8 text-center">Image Clarity</TableHead>
            <TableHead className="w-1/8 text-center">Optimal Sizing</TableHead>
            <TableHead className="w-1/8 text-center">Correct Image</TableHead>
        ...
        <TableBody>
          {Object.entries(groupedData).map(([platformKey, platformGroup]) => {
            ...
            const platformScore = calculateHeroImageScores(allSKUs);
            ...
            return (
              <React.Fragment key={platformKey}>
                ...
                {expandedItems.has(platformKey) && Object.entries(platformGroup.categories).map(([categoryKey, categoryGroup]) => {
                  ...
                  const categoryScore = calculateHeroImageScores(categorySKUs);
                  ...
                  return (
                    <React.Fragment key={fullCategoryKey}>
                      ...
                      {expandedItems.has(fullCategoryKey) && Object.entries(categoryGroup.brands).map(([brandKey, brandGroup]) => {
                        const brandScore = calculateHeroImageScores(brandGroup.products);
                            ...
                            {expandedItems.has(fullBrandKey) && brandGroup.products.map((product: SKUData) => (
                              <ProductRow
                                ...
                                product={{
                                  ...
                                  overallTitle: product.heroImage.score.overall,
                                  length: product.heroImage.imageClarity === 'clear' ? 100 : product.heroImage.imageClarity === 'moderate' ? 70 : 40,
                                  structure: product.heroImage.optimalSizing ? 100 : 50,
                                  searchTerms: product.heroImage.isCorrectImage ? 100 : 0,
                                  skuCount: 1
                                }}
                                onDrillDown={onDrillDown}
                                tabType="heroImage"
