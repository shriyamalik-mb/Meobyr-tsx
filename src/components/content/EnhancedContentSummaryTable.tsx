...
interface EnhancedContentSummaryTableProps {
  data: SKUData[];
  filters?: FilterState;
...
export const EnhancedContentSummaryTable = ({ data, filters, onDrillDown }: EnhancedContentSummaryTableProps) => {
  ...
  const calculateEnhancedContentScores = (skus: SKUData[]) => {
    const overallTitleScore = Math.round(skus.reduce((sum, sku) => sum + sku.enhancedContent.score.overall, 0) / skus.length);
    const lengthScore = Math.round(skus.reduce((sum, sku) => sum + (sku.enhancedContent.hasAPlusContent ? 100 : 0), 0) / skus.length);
    const structureScore = Math.round(skus.reduce((sum, sku) => sum + (sku.enhancedContent.hasVideo ? 100 : 0), 0) / skus.length);
    const searchTermsScore = Math.round(skus.reduce((sum, sku) => sum + (sku.enhancedContent.secondaryImagesCount > 0 ? 100 : 0), 0) / skus.length);
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            <TableHead className="w-1/8 text-center">A+ Content</TableHead>
            <TableHead className="w-1/8 text-center">Video</TableHead>
            <TableHead className="w-1/8 text-center">Images</TableHead>
        ...
        <TableBody>
          {Object.entries(groupedData).map(([platformKey, platformGroup]) => {
            ...
            const platformScore = calculateEnhancedContentScores(allSKUs);
            ...
            return (
              <React.Fragment key={platformKey}>
                ...
                {expandedItems.has(platformKey) && Object.entries(platformGroup.categories).map(([categoryKey, categoryGroup]) => {
                  ...
                  const categoryScore = calculateEnhancedContentScores(categorySKUs);
                  ...
                  return (
                    <React.Fragment key={fullCategoryKey}>
                      ...
                      {expandedItems.has(fullCategoryKey) && Object.entries(categoryGroup.brands).map(([brandKey, brandGroup]) => {
                        const brandScore = calculateEnhancedContentScores(brandGroup.products);
                            ...
                            {expandedItems.has(fullBrandKey) && brandGroup.products.map((product: SKUData) => (
                              <ProductRow
                                ...
                                product={{
                                  ...
                                  overallTitle: product.enhancedContent.score.overall,
                                  length: product.enhancedContent.hasAPlusContent ? 100 : 0,
                                  structure: product.enhancedContent.hasVideo ? 100 : 0,
                                  searchTerms: product.enhancedContent.secondaryImagesCount > 0 ? 100 : 0,
                                  skuCount: 1
                                }}
                                onDrillDown={onDrillDown}
